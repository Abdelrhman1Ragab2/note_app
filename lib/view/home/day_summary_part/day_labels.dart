...
class DayLabels extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<dynamic> dSummaryList =
        Provider.of<DSummaryProvider>(context).dSummaryBox.values.toList();
  ...
  Widget buildBody(
      BuildContext context, int dayNumbers, int monthNumber, dSummaryList) {
  ...
  Widget listBody(
      BuildContext context, int day, int monthNumber, dSummaryList) {
    bool currentDay =
        day == DateTime.now().day && monthNumber == DateTime.now().month;
    DSummary? dSummary =
        filteringDaySummary(context, dSummaryList, day, monthNumber);
    return InkWell(
      onTap: () {
        showDialog(
            context: context,
      ...
      onDoubleTap: (){
           if(dSummary!=null){
             Provider.of<DSummaryProvider>(context,listen: false).deleteDSummaryByKey(dSummary.id);
  ...
  DSummary? filteringDaySummary(
      BuildContext context, List<dynamic> dSummary, int day, int month) {
    ...
    if (newDaySummary != null) {
      Provider.of<DSummaryProvider>(context, listen: false)
          .dialogController
          .text = newDaySummary.text;
    }
